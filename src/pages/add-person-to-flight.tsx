...
import { getCitizen, getFlight, addPersonToFlight } from "../services/blockchain";
...
const AddPersonToFlight: React.FC = () => {
  ...
  const [transactionValues, setTransactionValues] = React.useState<BlockChainTransactionState>({
    transactionHash: "",
    transactionIndex: 0,
    blockHash: "",
    blockNumber: 0,
  ...
  const handleFormSubmit = (event: React.FormEvent<HTMLFormElement>, category: string) => {
  ...
  const checkIllegalTravel = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    event.preventDefault();
    ...
    return (
      <Container sx={{ padding: 5 }}>
            ...
            >
              {values.flight_error !== null ? (
                values.flight_error === true ? (
                  ...
                  <Alert severity="success">Success — Flight fetched successfully!</Alert>
                )
              ) : null}
            ...
            >
              {values.citizen_error !== null ? (
                values.citizen_error === true ? (
                  ...
                  <Alert severity="success">Success — User fetched successfully!</Alert>
                )
              ) : null}
              {values.citizen_error !== null && values.citizen_error !== true ? (
                <div>
                  <List>
                    ...
                    <ListItem>Passport Number: {citizenValues.passport_num}</ListItem>
                    <ListItem>Nationality: {citizenValues.nationality}</ListItem>
                    <ListItem>Last Known Location: {citizenValues.last_known_location}</ListItem>
                  </List>
                </div>
              ) : null}
            </Card>
        ...
        <Stack>
          <Button
            disabled={
              values.flight_error === false && values.citizen_error === false ? false : true
            }
            variant="outlined"
          ...
          {values.illegal_travel !== null ? (
            values.illegal_travel === true ? (
              ...
              <div>
                ...
                <List>
                  <ListItem>transactionHash: {transactionValues.transactionHash}</ListItem>
                  <ListItem>transactionIndex: {transactionValues.transactionIndex}</ListItem>
                  <ListItem>blockHash: {transactionValues.blockHash}</ListItem>
                  <ListItem>blockNumber: {transactionValues.blockNumber}</ListItem>
                  <ListItem>contractAddress: {transactionValues.contractAddress}</ListItem>
                  <ListItem>cumulativeGasUsed: {transactionValues.cumulativeGasUsed}</ListItem>
